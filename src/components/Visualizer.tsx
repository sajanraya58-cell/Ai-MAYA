...
export default function Visualizer({ state, name = "MAYA", persona = "maya" }: VisualizerProps) {
  ...
  // Color palette based on assistant persona
  const getTheme = () => {
    if (persona === "maya") {
      switch (state) {
        case "listening": return { color: "rgba(236, 72, 153, 1)", glow: "shadow-pink-500/70", border: "border-pink-400" };
        case "processing": return { color: "rgba(56, 189, 248, 1)", glow: "shadow-sky-400/80", border: "border-sky-400" };
        case "speaking": return { color: "rgba(244, 63, 94, 1)", glow: "shadow-rose-500/80", border: "border-rose-400" };
        default: return { color: "rgba(20, 184, 166, 0.85)", glow: "shadow-teal-500/50", border: "border-teal-500/60" };
      }
    } else {
      switch (state) {
    ...
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none">
      ...
      >
        ...
        <motion.div 
          ...
          className="font-bold tracking-[0.3em] text-xl md:text-3xl lg:text-4xl text-white select-none"
